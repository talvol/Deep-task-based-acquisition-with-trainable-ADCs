...
p = 7  # Number of ADCs
noise_sigma = 0.3
beta = 280  # Power accuracy trade-off
...
if distillation_training:
    ...
    filename = f"Teacher_MNIST_0_0_{q_bits}_{p}.pth"
...
class digitalDNN(nn.Module):
    def __init__(self, num_of_adcs):
        ...
        self.L2 = torch.nn.Linear(num_of_adcs * 2, 10, bias=True).float()
...
def calcLoss(output_teacher, output, labels, Vin, W_tensor, Q):
    ...
    if distillation_training:
        ...
        loss = CEloss(output, labels) + alpha * dist_loss + beta * totpower + gamma * level_merg_error + eta * l2_reg
       # print("Distillation Loss, ", alpha * dist_loss)
    else:
        loss = CEloss(output, labels) + beta * totpower + gamma * level_merg_error + eta * l2_reg
...
def test(network, batch_size, test_loader, x_test, s_test):
    ...
    for r in range(num_runs):
        # Calc accuracy
        test_out, Q, Vin_t, W_t = test_net(torch.FloatTensor(x_test))
